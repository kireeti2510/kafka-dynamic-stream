"""
Database Setup Module - Distributed Version
...
- Database path can be configured via config.json
- Supports network-mounted database via NFS
...
_default_db_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'topics.db')

# Global DB_PATH that can be overridden
DB_PATH = _default_db_path

def set_db_path_from_config(config_path='config.json'):
    """
    Set database path from configuration file.
    ...
    if os.path.exists(config_file):
        try:
            ...
            print(f"⚠ Could not read db_path from config: {e}")
...
def initialize_database():
    ...
    """
    # Try to load DB path from config first
    set_db_path_from_config()
